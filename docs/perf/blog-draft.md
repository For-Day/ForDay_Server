...
## Step 5: 실측으로 찾은 문제 2개 수정 후 재측정

Step 1~4를 측정하는 과정에서 의도치 않게 두 가지를 더 발견·수정했다.

1. **`record_reaction_count` 데드락 버그**: Step 1 코드 분석 때 짚었던 "카운트 UPDATE
   후 영향받은 행이 0이면 INSERT" 패턴이, 동시 요청 상황에서 실제로 MySQL 데드락을
   일으키는 걸 로그로 확인했다(`Deadlock found when trying to get lock`). `INSERT ...
   ON DUPLICATE KEY UPDATE` 원자적 upsert로 바꿔 check-then-insert 경쟁 상태 자체를
   없앴다.
2. **RDS 스케일업**: 4단계 내내 여유 메모리가 100~127MB(db.t4g.micro 1GB 중)로 빠듯했던
   걸 보고 `db.t4g.small`(2GB)로 올렸다.

### 1. 측정 결과 해석 — 콜드 상태(적용 직후)

| 지표 | run1 | run2 | run3 | **중위값** |
| --- | --- | --- | --- | --- |
| Raw TPS | 247.3 | 255.1 | 307.2 | **255.1** |
| 보정 TPS | 233.0 | 239.5 | 288.0 | **239.5** |
| Request Duration p95 | 4,155ms | 3,848ms | 3,356ms | **3,848ms** |

Step 4(399.7 / 381.5)보다 오히려 낮게 나왔다 — RDS 인스턴스 클래스 변경은 인스턴스
재시작을 동반해 InnoDB 버퍼풀이 비어있는 콜드 캐시 상태였고, 앱 컨테이너도 방금
재배포해서 JVM·HikariCP가 워밍업되지 않은 상태였다.

### 2. 워밍업 후 재측정

40초 대시보드 러닝으로 워밍업한 뒤 다시 3회 측정했다.

| 지표 | run1 | run2 | run3 | **중위값** |
| --- | --- | --- | --- | --- |
| Raw TPS | 364.0 | 386.1 | 428.0 | **386.1** |
| 보정 TPS | 345.7 | 333.9 | 394.9 | **345.7** |
| Request Duration p95 | 2,970ms | 2,744ms | 2,459ms | **2,744ms** |

콜드 상태보다는 크게 회복했지만(255.1→386.1), **Step 4(399.7/381.5)와 비교하면
여전히 비슷하거나 소폭 낮다.**

### 3. 해석 — 왜 개선이 안 보였나

`(직접 채우기 — 예시: 두 수정 모두 Step 4(v2/분산 락 경로)의 실제 병목과는 거리가
있었다고 해석한다. record_reaction_count 데드락은 동기 경로(v1의 reactToRecord)에서만
발생하는 패턴이고, v2는 카운트 반영을 스케줄러의 델타 UPDATE(increaseCountBy)로
처리해 애초에 이 경쟁 상태에 해당하지 않는다. RDS 스케일업도 RDS CPU가 4단계 내내
20~30%로 여유로웠던 걸 감안하면, RDS가 v2 경로의 진짜 병목이 아니었다는 뜻이다.
즉 "고쳤다고 항상 빨라지진 않는다" — 실측으로 병목을 특정하지 않고 짐작으로 고치면
이렇게 헛다리를 짚을 수 있다는 것 자체가 이번 라운드의 결론이다.)`

이 결과는 Step 4의 진짜 병목이 EC2 CPU나 애플리케이션 처리 쪽(예: `getValidRecord`/
`validateAccess` 동기 DB 호출)에 더 가깝다는 이전 관찰과 일치한다 — 다음으로 시도해볼
레버는 그쪽이다.
...
| ⑤ 데드락 수정 + RDS 스케일업 | 386.1(워밍업 후) | 345.7(워밍업 후) | 2,744ms | 낮음 | - | Step 4 대비 개선 없음 — 병목이 다른 곳에 있었다는 증거 |
...
- 이번 측정 중 별도로 발견한 사전 존재 버그: `record_reaction_count` 초기 행 생성(`increaseCount` 실패 시 `save`)이 동시 요청 시 MySQL 데드락을 일으킨다(check-then-insert 경쟁 상태) — 이번 이슈 범위 밖이라 별도 이슈로 남긴다.
